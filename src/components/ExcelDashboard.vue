...
function sortRows() {
  ...
  const body = activeSheet.value.data.slice(DATA_START_ROW)
...
function dropRow(targetIndex) {
  const realFrom = DATA_START_ROW + dragIndex.value
  const realTo = DATA_START_ROW + targetIndex

  const data = activeSheet.value.data
  const moved = data.splice(realFrom, 1)[0]
  data.splice(realTo, 0, moved)
